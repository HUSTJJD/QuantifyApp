...
/** 滚动标准差（总体标准差）。period<=0 或数据不足返回 NaN。 */
export function stdev(values: number[], period: number): number[] {
  ...
  for (let i = period - 1; i < n; i++) {
    let mean = 0;
    for (let w = i - period + 1; w <= i; w++) mean += values[w];
    mean /= period;
    let variance = 0;
    for (let w = i - period + 1; w <= i; w++) {
      const d = values[w] - mean;
      variance += d * d;
    }
...
/**
 * 布林带。返回 { mid, upper, lower }，默认(20, 2)。
 * mid 为中轨(SMA)，上下轨 = mid ± k * 滚动标准差。
 */
export function bollinger(
  ...
  const upper = values.map((_, i) =>
    Number.isNaN(mid[i]) || Number.isNaN(sd[i]) ? NaN : mid[i] + k * sd[i],
  );
