...
export interface CandidateScoreOptions {
  ...
  /** 理由覆盖度权重（默认 30） */
  reasonWeight?: number;
  /** 外部信号强度权重（默认 30），需配合 strengthByKey 传入 */
  strengthWeight?: number;
}

/** 理由覆盖度：以「、」/「,」分隔的论点数量归一化到 0~1（≥3 条视为满分） */
export function reasonCoverage(reasons: string): number {
  if (!reasons) return 0;
  const parts = reasons
    .split(/[、,]/)
    .map((s) => s.trim())
    .filter(Boolean);
  return Math.min(1, parts.length / 3);
}

/**
 * 候选综合打分（0~100）：涨跌幅幅度 + 理由覆盖度 + 可选信号强度。
 * 纯函数，供 UI 排序与高亮复用，避免各处重复排序逻辑。
 *
 * @param strengthByKey 可选，symbolKey(`code.exchange`) → 信号强度，用于加权
 */
export function scoreCandidate(
  item: CandidateItem,
  opts: CandidateScoreOptions = {},
  strengthByKey?: Map<string, number>,
): number {
  const changeWeight = opts.changeWeight ?? 40;
  const reasonWeight = opts.reasonWeight ?? 30;
  const strengthWeight = opts.strengthWeight ?? 30;

  const chg = item.changePct ?? 0;
  // tanh 压缩涨跌幅，避免极端值主导；±5% 对应约 ±38 分
  const changeScore = Math.tanh(Math.abs(chg) / 5) * changeWeight;
  const reasonScore = reasonCoverage(item.reasons) * reasonWeight;

  let strengthScore = 0;
  if (strengthByKey && strengthWeight > 0) {
    const k = `${item.symbol.code}.${item.symbol.exchange}`;
    const s = strengthByKey.get(k);
    if (s != null) strengthScore = Math.min(1, Math.abs(s) / 3) * strengthWeight;
  }

  const raw = changeScore + reasonScore + strengthScore;
