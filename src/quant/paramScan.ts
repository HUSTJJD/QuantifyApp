 ...
 */
import type { ParamSpec, StrategyProfile, ProfileEvaluator } from './profile';
import { profileEvaluator, profileWithParams } from './profile';
...
/** 用一组扁平参数覆盖所有腿的因子参数，返回可回测的 evaluate 函数。 */
export function strategyWithParams(profile: StrategyProfile, params: ParamCombo): ProfileEvaluator {
  return profileEvaluator(profileWithParams(profile, params));
}
