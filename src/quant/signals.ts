...
export interface TradeSignal {
  ...
  reasons: string[];
  /** 各策略加权明细（策略 id → 加权贡献），便于调试与 UI 展示 */
  contributions: { id: string; side: SignalSide; strength: number; weight: number }[];
  /** 信号生成时间（ms） */
  ts: number;
}
...
export function computeSignal(
  ...
  const parts: { id: string; side: SignalSide; reason: string; strength: number; weight: number }[] = [];
  ...
  for (const s of activeStrategies(config)) {
    try {
      ...
      if (p) {
        ...
        parts.push({ id: s.id, side: p.side, reason: p.reason, strength: p.strength, weight: w });
      }
    } catch {
      // 单策略异常不影响整体（如数据不足），跳过
    }
  ...
  const contributions: { id: string; side: SignalSide; strength: number; weight: number }[] = [];
  for (const p of parts) {
    score += p.strength * p.weight;
    reasons.push(p.reason);
    contributions.push({ id: p.id, side: p.side, strength: p.strength, weight: p.weight });
  }
