...
export interface BacktestSeries {
  ...
  /** 参与回测的 bar 数（UI 不要自己猜，直接用它展示） */
  barCount: number;
  /** 实际区间（ms）；无数据为 null */
  rangeMs: { start: number; end: number } | null;
  /**
   * 请求周期是否被真实满足。
   * 本地库当前只按日 K 存储，分钟级请求会由 `aggregateByPeriod` 退化成日 K，
   * 此时为 false——UI 必须提示，否则用户会以为跑的是分钟级策略。
   */
...
/** 分钟级周期（本地库暂不存储，会退化成日 K） */
const INTRADAY_PERIODS: readonly KlinePeriod[] = ['1m', '5m', '15m', '30m', '60m'];

export interface LoadBacktestSeriesOptions extends LoadCandlesOptions {
  /**
   * 回测窗口上限：只保留最近 N 根 bar。
   *
   * 不传则本地路径返回库里**全部**历史（不同用户同步深度不同 → 回测样本不一致），
   * 网络路径也只受 `count × maxPages` 松散约束。想要「同一份样本可复现」请显式传入
   * （UI 的 0.5/1/3 年切换即走这里）。
   */
  maxBars?: number;
  /**
   * 复权模式：
...
export async function loadBacktestSeries(
  ...
  // 回测窗口：显式 maxBars 时只保留最近 N 根，保证同一样本可复现
  const maxBars = opts.maxBars && opts.maxBars > 0 ? Math.floor(opts.maxBars) : 0;
  const capped = maxBars > 0 && preparedRaw.candles.length > maxBars
    ? preparedRaw.candles.slice(-maxBars)
    : preparedRaw.candles;
  const base = capped;

  // 本地库只存日 K → 分钟级请求实际拿到的是日 K，必须如实告知
  const periodHonored = !(source === 'local' && period !== 'day' && INTRADAY_PERIODS.includes(period));
  ...
  const rangeMs =
    barCount > 0
      ? { start: timeOf(candles[0]), end: timeOf(candles[barCount - 1]) }
      : null;
  ...
  const degradeNote =
    periodHonored || barCount === 0
      ? ''
      : ` · 注意：本地库仅存日K，${period} 已退化为日K回放`;
  const windowNote =
    barCount > 0 && rangeMs
      ? ` · 样本 ${barCount} 根（${fmtDate(rangeMs.start)} ~ ${fmtDate(rangeMs.end)}）`
      : '';
  ...
  return {
    ...
    rangeMs,
    periodHonored,
    note: `${note}${degradeNote}${windowNote}`,
  };
}
...
export async function runBacktestOnSymbol(
  ...
  return runBacktest(evaluate, prepared.candles, {
    ...opts,
