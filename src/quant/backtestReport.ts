...
import { PERIOD_LABELS, SESSION_LABELS } from './profile';
...
export function formatDetailSection(d: DetailExport): string {
  ...
  const lines: string[] = [
    ...
    `胜率 ${x.winRate.toFixed(0)}% · 盈亏比 ${Number.isFinite(x.profitFactor) ? x.profitFactor.toFixed(2) : '∞'} · 总费用 ${num(x.totalFees, 0)} · 成交 ${x.trades.length} 笔`,
  ...
  if (recent.length > 0) {
    ...
    for (const t of recent) {
      lines.push(
        `${date(t.time)} ${t.side === 'buy' ? '买' : '卖'} ${t.shares}股 @${t.price.toFixed(2)} 费${t.fee.toFixed(2)}`,
      );
    }
  }
  return lines.join('\n');
