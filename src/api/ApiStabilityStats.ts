...
/** 方法键：强类型（与 DataSourceMethod 一致），避免字符串拼错时统计悄悄失真 */
export type MethodKey = DataSourceMethod;
...
export interface SourceStat {
  ...
  /** 各方法是否曾经成功过（覆盖度）：稀疏映射，仅含真正尝试过的方法 */
  methodsOk: Partial<Record<MethodKey, boolean>>;
  /** 各方法真实失败次数（用于展示“试过但挂了”的栏目；“能力不支持”不计入）：稀疏映射 */
  methodFails: Partial<Record<MethodKey, number>>;
}
...
function methodLabel(method: MethodKey): string {
  const map: Partial<Record<DataSourceMethod, string>> = {
    getKline: 'K线',
    getIndexKline: '指数K线',
    getQuotes: '行情',
    getIndexQuotes: '指数行情',
...
export class ApiStabilityStats {
  ...
  failure(sourceId: SourceId, method: MethodKey, err: unknown): void {
    ...
    if (method) s.methodFails[method] = (s.methodFails[method] ?? 0) + 1;
  }
