...
export interface KlineDatabasePort {
  ...
  /** 物理删除某标的某周期在 [fromTs, toTs] 闭区间内的 K 线（用于区间清理，替代粗粒度 prune） */
  deleteRange(
    symbol: Symbol,
    period: KlinePeriod,
    fromTs: number,
...
export class KlineDatabase {
  ...
  async saveCandles(
    ...
    // 防无限堆积：单次入参超过上限时先截断为最近 N 根（追溯上市场景远小于上限，不受影响）
    const sliced =
      candles.length > KLINE_MAX_ROWS_PER_SERIES
        ? candles.slice(-KLINE_MAX_ROWS_PER_SERIES)
        : candles;
    const rows = candlesToRows(symbol, period, sliced);
    ...
    if (cached) {
      ...
      // 内存无全量视图，回读落盘以保证热索引用的是权威数据；超出上限同样截断
      const fresh = await this.port.getSeries(symbol, period);
      this.index.set(symbol, period, fresh.slice(-KLINE_MAX_ROWS_PER_SERIES));
    }
    return written;
  }
  ...
  async getLatestCandles(
    ...
    // 命中热索引失败时只回读这一次：limit 未给 / 返回行数不足 limit，
    // 说明拿到的就是完整序列，可直接回填（旧实现会再全量 getSeries 一次，多一次落盘读）。
    const isFullSeries = !limit || rows.length < limit;
    if (isFullSeries) {
      this.index.set(symbol, period, rows.slice(-KLINE_MAX_ROWS_PER_SERIES));
    }
    return rowsToCandles(rows);
  }
  ...
  /** 区间删除：物理清理某标的某周期 [fromTs, toTs] 内的 K 线，并失效对应热索引 */
  async deleteCandlesInRange(
    symbol: Symbol,
    period: KlinePeriod,
    fromTs: number,
    toTs: number,
    ...
    if (this.closed) return 0;
    const n = await this.port.deleteRange(symbol, period, fromTs, toTs);
    this.index.invalidate(symbol, period);
    return n;
  }
