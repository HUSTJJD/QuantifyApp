            ...
            <>
              <View style={[styles.quickRow, { marginTop: spacing.sm, marginBottom: 0 }]}>
                {REGIME_PRESETS[regime.regime].templateIds.map((tid) => {
                  const t = STRATEGY_TEMPLATES.find((s) => s.id === tid);
                  if (!t) return null;
                  return (
                    <TouchableOpacity
                      key={tid}
      ...
      <Section
        ...
        action={
          <TouchableOpacity onPress={onCreate} activeOpacity={0.7}>
            <Text style={styles.addText}>+ 新增策略</Text>
          </TouchableOpacity>
        }
      />
    ...
    <Card style={styles.strategyCard} padded={false}>
      ...
      <View style={styles.cardMetaWrap}>
        <Text style={styles.cardMeta} numberOfLines={2}>
          {profileLegsSummary(profile)} · {COMBINE_LABEL[profile.combineMode]} · {UNIVERSE_LABEL[profile.selection.universe] ?? '自选股'} · {PERIOD_LABELS[profile.trade.period]} · {SESSION_LABELS[profile.trade.session]} · 仓位
