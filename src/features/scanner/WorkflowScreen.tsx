...
export function WorkflowScreen({
  ...
  const [pool, setPool] = useState<RankedCandidate[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const candidatePool = rankCandidatePool(buildCandidatePool(hitRows));
                      ...
                      <View style={{ alignItems: 'center', marginRight: 8 }}>
                        <Text
                          style={{
                            fontSize: 13,
                            fontWeight: '700',
                            color: item.score >= 70 ? c.up : item.score >= 40 ? c.warning : c.textSecondary,
                          }}
                        >
                          {item.score}
                        </Text>
                        <Text style={{ fontSize: 9, color: c.textSecondary }}>评分</Text>
                      </View>
