...
export function SettingsScreen({
  ...
  const [lpKey, setLpKey] = useState('');
  const [lpSecret, setLpSecret] = useState('');
  ...
  useEffect(() => {
    ...
    import('@/data/api/sources/LongportSource')
      .then(async (m) => {
        const cred = await m.loadLongportCreds();
        const fill = cred ?? m.DEFAULT_LONGPORT_CREDS;
        setLpKey(fill.appKey);
        setLpSecret(fill.appSecret);
        setLpToken(fill.accessToken);
      })
      .catch(() => undefined);
  }, [loadSettings]);
  ...
  return (
    ...
    >
      ...
      <Text style={styles.sectionTitle}>长桥 OpenAPI（港美）</Text>
      <Text style={styles.sectionHint}>
        用于港美行情（需开通 OpenAPI）。npm 包 longbridge 为 Node 原生扩展，App 内走 HTTP 签名。
      </Text>
      <TextInput
        style={styles.input}
        placeholder="App Key"
      ...
      <TextInput
        ...
        placeholderTextColor={colors.textSecondary}
        value={lpToken}
        onChangeText={setLpToken}
        autoCapitalize="none"
        autoCorrect={false}
        secureTextEntry
      />
      <TouchableOpacity
        style={[styles.btn, (saving || !lpKey.trim() || !lpSecret.trim() || !lpToken.trim()) && styles.btnDisabled]}
        disabled={saving || !lpKey.trim() || !lpSecret.trim() || !lpToken.trim()}
        onPress={async () => {
          setSaving(true);
          try {
            const { setLongportCreds } = await import('@/data/api/sources/LongportSource');
            setLongportCreds({
              appKey: lpKey.trim(),
              appSecret: lpSecret.trim(),
              accessToken: lpToken.trim(),
            });
            Alert.alert('已保存', '长桥凭证已保存，港美行情源将优先于 dukascopy');
          } finally {
            setSaving(false);
          }
        }}
      >
        <Text style={styles.btnText}>保存长桥凭证</Text>
      </TouchableOpacity>
