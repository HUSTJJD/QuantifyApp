...
export interface ReconcileReport {
  /** 现金是否与流水勾稽一致（cash == initCash + ΣcashDelta） */
  consistent: boolean;
  /** 当前可用现金 */
  cash: number;
  /** 持仓总成本（costPrice * shares） */
  positionCost: number;
  /** 累计已支付费用（所有 trade.fee 之和） */
  totalFee: number;
  /** 现金勾稽漂移 = cash - (initCash + ΣcashDelta)，应≈0 */
  drift: number;
  /** 流水笔数（trades）与订单成交笔数是否对得上 */
  tradeOrderConsistent: boolean;
  issues: string[];
}

/**
 * 对账：校验账户内部一致性。用于模拟盘结算/审计，确保现金与流水勾稽、订单/流水一致、持仓整手。
 * 恒等式：账户现金 = 初始资金 + 所有成交的现金影响之和（cashDelta 买入为负、卖出为正）。
 * 纯函数。
 */
export function reconcile(account: SimAccount): ReconcileReport {
  const issues: string[] = [];
  const cash = round2(account.cash);
  const positionCost = round2(
    account.positions.reduce((s, p) => s + p.costPrice * p.shares, 0),
  );
  const totalFee = round2(account.trades.reduce((s, t) => s + t.fee, 0));
  const cashFromTrades = round2(account.trades.reduce((s, t) => s + t.cashDelta, 0));
  // 现金 = 初始资金 + 累计流水现金影响
  const drift = round2(cash - (account.initCash + cashFromTrades));
  if (Math.abs(drift) > 0.01) {
    issues.push(`现金勾稽漂移 ${drift} 元（cash≠initCash+ΣcashDelta）`);
  }
  // 成交笔数与订单 filled 数应一致
  const filledOrders = account.orders.filter((o) => o.status === 'filled').length;
  const tradeCount = account.trades.length;
  const tradeOrderConsistent = filledOrders === tradeCount;
  if (!tradeOrderConsistent) {
    issues.push(`成交流水 ${tradeCount} 笔 ≠ 已成交订单 ${filledOrders} 笔`);
  }
  // 持仓股数必须为整手
  const badLot = account.positions.find((p) => p.shares % LOT_SIZE !== 0);
  if (badLot) {
    issues.push(`持仓 ${badLot.symbol.code} 股数非整手`);
  }
  return {
