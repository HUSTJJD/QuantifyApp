#!/usr/bin/env node
/**
 * postinstall：确保 FuyaoNPM 子模块的 dist 产物存在（见 package.json "@opptrix/fuyao": "file:./FuyaoNPM"）。
 *
 * 背景：FuyaoNPM 是 git 子模块，其 package.json 的 main/exports 指向 dist/*，
...
const sdkDir = path.join(__dirname, '..', 'FuyaoNPM');
...
if (!fs.existsSync(path.join(sdkDir, 'package.json'))) {
  ...
  console.warn(
    '[build-fuyao-sdk] FuyaoNPM/ 缺少 package.json（子模块未检出？请先 git submodule update --init --recursive），跳过构建'
  );
  process.exit(0);
}

if (fs.existsSync(distMain)) {
  console.log('[build-fuyao-sdk] FuyaoNPM dist 已存在，跳过构建');
  process.exit(0);
}

console.log('[build-fuyao-sdk] FuyaoNPM dist 缺失，开始构建（首次安装/EAS 构建需要，约 1-3 分钟）…');

if (!fs.existsSync(path.join(sdkDir, 'node_modules', '.bin', 'tsup'))) {
  console.log('[build-fuyao-sdk] FuyaoNPM 依赖未安装，执行 npm install …');
...
if (!fs.existsSync(distMain)) {
  console.error('[build-fuyao-sdk] 构建完成但未找到 dist/index.cjs，请检查 FuyaoNPM 构建日志');
  process.exit(1);
}

console.log('[build-fuyao-sdk] FuyaoNPM 构建完成 ✓');
