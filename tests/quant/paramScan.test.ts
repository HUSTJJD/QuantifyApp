...
import { paramGroupsOf, paramSpecsOf, createProfile } from '@/quant/profile';
...
describe('strategyWithParams / formatParamCombo', () => {
  it('构造的 strategy 可 evaluate 且参数覆盖生效', () => {
    const profile = createProfile(['trend_confirm']);
    const evaluate = strategyWithParams(profile, { 'ma_cross.fast': 3 });
