...
function profileOf(): StrategyProfile {
  return {
    ...
    enabled: true,
    autoTrade: false,
    legs: [{ templateId: 'ma_cross', enabled: true, weight: 1, params: {} }],
    combineMode: 'and',
...
describe('formatDetailSection', () => {
  ...
  it('提供买入持有时输出对照行，明确同口径', () => {
    ...
    const without = formatDetailSection(base);
    expect(without).not.toContain('vs 买入持有');

    const withBh = formatDetailSection({
      ...base,
      buyHold: {
        totalReturnPct: 12,
        equity: [100_000, 112_000],
        shares: 900,
        buyPrice: 10.5,
        buyFee: 23.4,
        corpEvents: [],
    ...
    expect(withBh).toContain('vs 买入持有');
    expect(withBh).toContain('+12.00%');
    expect(withBh).toContain('-11.00%');
    expect(withBh).toContain('900股');
  });
});
