...
describe('appPrefs', () => {
  ...
  it('无存储时返回默认', async () => {
    ...
    expect(p.homeFocusIndex).toBe('000001.SH');
    expect(p.ashareFocusTab).toBe('surge');
  });
  ...
  it('布尔开关可关', async () => {
    const p = await setAppPrefs({ homeFundsCollapsed: true, notifyPriceAlert: false });
    expect(p.homeFundsCollapsed).toBe(true);
    expect(p.notifyPriceAlert).toBe(false);
  });
