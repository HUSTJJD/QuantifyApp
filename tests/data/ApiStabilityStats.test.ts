...
describe('ApiStabilityStats', () => {
  ...
  it('覆盖率统计：仅记录曾经成功的 method', () => {
    stats.success('fuyao', 'getQuotes', stats.begin('fuyao', 'getQuotes'));
    stats.failure('fuyao', 'getKline', new DataSourceError('缺能力', 'fuyao'));
    const s = stats.getStat('fuyao')!;
  ...
  it('排序：成功率高的源排在前', () => {
    stats.success('good', 'getQuotes', stats.begin('good', 'getQuotes'));
    stats.failure('bad', 'getQuotes', new DataSourceError('x', 'bad'));
    const all = stats.getAll();
    expect(all[0].id).toBe('good');
  });

  it('失败原因归一化：超时/网络/不支持被归类', () => {
    stats.failure('s', 'getQuotes', new DataSourceError('请求超时(1000ms)', 's'));
    stats.failure('s', 'getQuotes', new DataSourceError('fetch failed', 's'));
    stats.failure('s', 'getQuotes', new DataSourceError('msg', 's', 1002));
