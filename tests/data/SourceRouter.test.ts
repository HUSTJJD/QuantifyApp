...
describe('SourceRouter —— 瞬态失败 + 他源如实返回空 → 降级无数据（LogBox 刷屏修复）', () => {
  ...
  it('429(可重试) + 他源如实返回空 → resolve 空数组，不打 console.error', async () => {
    const r = twoSrcRouter(makeFlakySrc('fuyao', 'throw429'), makeFlakySrc('stock-sdk', 'empty'));
  ...
  it('持久故障(3001) + 他源返回空 → 仍抛聚合错误并 console.error', async () => {
    const r = twoSrcRouter(makeFlakySrc('fuyao', 'throw3001'), makeFlakySrc('stock-sdk', 'empty'));
    const errSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
    try {
