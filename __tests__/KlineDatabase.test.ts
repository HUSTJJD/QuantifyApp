...
describe('KlineDatabase 行级存储', () => {
  ...
  it('追溯上市：超过旧 2000 根上限的历史全部保留（上限 10000）', async () => {
    const db = makeDb();
    const N = 2500; // 旧上限 2000 会截断，新上限 10000 应全部保留
    const many: Candle[] = Array.from({ length: N }, (_, i) => candle(i, 100 + i));
    await db.saveCandles(SYM, PERIOD, many);
    const rows = await db.getCandles(SYM, PERIOD);
    expect(rows).toHaveLength(N); // 追溯上市数据不被截断
    expect(rows[0].close).toBe(100);
    expect(rows[N - 1].close).toBe(100 + N - 1);
  });

  it('超过上限时仍截断保留最近 N 根（防无限堆积）', async () => {
    const db = makeDb();
    const N = KLINE_MAX_ROWS_PER_SERIES + 500;
    const many: Candle[] = Array.from({ length: N }, (_, i) => candle(i, i));
    await db.saveCandles(SYM, PERIOD, many);
    const rows = await db.getCandles(SYM, PERIOD);
    expect(rows).toHaveLength(KLINE_MAX_ROWS_PER_SERIES);
    expect(rows[0].close).toBe(N - KLINE_MAX_ROWS_PER_SERIES); // 保留最近 N 根
  });
