...
describe('getQuotes（跨市场批量行情）', () => {
  ...
  it('单市场失败不拖垮整批：cn 失败但 hk 成功时返回 hk 行情（部分成功）', async () => {
    ...
    expect(mockSdk.quotes.cn).toHaveBeenCalled();
    expect(mockSdk.quotes.hk).toHaveBeenCalledWith(['00700']);
    expect(res).toHaveLength(1);
    expect(res[0].last).toBe(400);
  });
  it('全部市场失败才整体抛 DataSourceError', async () => {
    mockSdk.quotes.cn.mockRejectedValue(new Error('CN 行情失败'));
    mockSdk.quotes.hk.mockRejectedValue(new Error('HK 行情失败'));
    const s = new StockSdkSource();
    await expect(s.getQuotes([CN('600519'), HK('00700')])).rejects.toBeInstanceOf(DataSourceError);
  });
  it('全部市场失败时，聚合错误应透出各市场原始根因（便于定位 stock-sdk 真实失败原因）', async () => {
    mockSdk.quotes.cn.mockRejectedValue(new Error('ECONNREFUSED 上游网关超时'));
    mockSdk.quotes.hk.mockRejectedValue(new Error('ETIMEDOUT 港股网关无响应'));
    const s = new StockSdkSource();
    await expect(s.getQuotes([CN('600519'), HK('00700')])).rejects.toThrow(
      /ECONNREFUSED 上游网关超时.*ETIMEDOUT 港股网关无响应/,
    );
  });
});
...
describe('getKline（历史K线）', () => {
  ...
  describe('BK 板块指数（东方财富行业/概念）', () => {
    const BK = (date: string) => ({ date, open: 10, high: 11, low: 9, close: 10.5, volume: 1000, amount: 10000 });
    it('BK 代码日 K 走 board.industry.kline，不再按个股 kline.cn 拉（否则必空）', async () => {
      mockSdk.board.industry.kline.mockResolvedValue([BK('2024-01-01'), BK('2024-01-02')]);
      const s = new StockSdkSource();
      const res = await s.getKline({ symbol: CN('BK0475'), period: 'day' });
      expect(mockSdk.kline.cn).not.toHaveBeenCalled();
      expect(mockSdk.board.industry.kline).toHaveBeenCalledWith('BK0475', { period: 'daily', adjust: '' });
      expect(res).toHaveLength(2);
      expect(res[0]).toMatchObject({ datetime: '2024-01-01', close: 10.5 });
    });
    it('周 K 透传起止时间（SDK BoardKlineOptions 无 limit）', async () => {
      mockSdk.board.industry.kline.mockResolvedValue([BK('2024-01-01')]);
      await new StockSdkSource().getKline({
        symbol: CN('BK0475'),
        period: 'week',
        count: 120,
      ...
      expect(mockSdk.board.industry.kline).toHaveBeenCalledWith('BK0475', {
        period: 'weekly',
        adjust: '',
        startDate: expect.any(String),
        endDate: expect.any(String),
    ...
    it('分钟 K 走 board.industry.minuteKline', async () => {
      mockSdk.board.industry.minuteKline.mockResolvedValue([{ time: 1, open: 10, high: 11, low: 9, close: 10.5, volume: 100 }]);
      const res = await new StockSdkSource().getKline({ symbol: CN('BK0475'), period: '1m' });
      expect(mockSdk.kline.cnMinute).not.toHaveBeenCalled();
      expect(mockSdk.board.industry.minuteKline).toHaveBeenCalledWith('BK0475', { period: '1' });
      expect(res).toHaveLength(1);
    });
    it('industry 无数据时回退 concept（BK 代码段行业/概念共用）', async () => {
      mockSdk.board.industry.kline.mockResolvedValue([]);
      mockSdk.board.concept.kline.mockResolvedValue([BK('2024-02-01')]);
      const res = await new StockSdkSource().getKline({ symbol: CN('BK9999'), period: 'day' });
      expect(mockSdk.board.industry.kline).toHaveBeenCalled();
      expect(mockSdk.board.concept.kline).toHaveBeenCalledWith('BK9999', { period: 'daily', adjust: '' });
      expect(res).toHaveLength(1);
    });
    it('行业+概念都无数据时返回空数组（不抛错，交给上层按无数据处理）', async () => {
      mockSdk.board.industry.kline.mockResolvedValue([]);
...
describe('指数 / 板块', () => {
   ...
   * spot 的真实返回是 KV 结构 { item, value }[]（item 为上游内部字段代号），
   * 不是对象数组；list 才给出 code/name/price/changePercent/totalMarketCap 全字段。
   * 这里按真实类型（IndustryBoard）构造 fixture，避免 mock 与线上结构脱节。
   */
  const BOARD: Record<string, unknown> = {
    ...
    fallCount: 5,
    leadingStock: '招商银行',
    leadingStockChangePercent: 3.1,
  };

  it('getBoardQuotes 走 board.industry.list（不用 spot：它是 KV 结构）', async () => {
    mockSdk.board.industry.list.mockResolvedValue([BOARD]);
    const s = new StockSdkSource();
    const res = await s.getBoardQuotes([CN('BK0001')]);
    expect(mockSdk.board.industry.list).toHaveBeenCalled();
    expect(mockSdk.board.industry.spot).not.toHaveBeenCalled();
    expect(res[0].last).toBe(1200);
    expect(res[0].changePct).toBe(1.2);
    // 本源不返回板块成交额，amount 用总市值代理
    expect(res[0].amount).toBe(1e12);
    // 板块无昨收，不要指望 prevClose
    expect(res[0].prevClose).toBe(0);
  });

  it('getStockIndustryBoard 映射板块全字段', async () => {
    mockSdk.board.industry.list.mockResolvedValue([BOARD, { ...BOARD, code: 'BK0002', name: '半导体', changePercent: -2.5 }]);
    const s = new StockSdkSource();
    const res = await s.getStockIndustryBoard({ limit: 1 });
...
describe('特色数据', () => {
  ...
  it('getLimitUpLadder 由涨停池合成单日板位矩阵', async () => {
    mockSdk.marketEvent.ztPool.mockResolvedValue([
      { code: 'A', name: 'A', continuousBoardCount: 1 },
      { code: 'B', name: 'B', continuousBoardCount: 5 },
      { code: 'C', name: 'C', continuousBoardCount: 2 },
    ]);
    const s = new StockSdkSource();
    const r = await s.getLimitUpLadder();
    expect(r.days).toHaveLength(1);
    expect(r.days[0].boards.five_board[0].symbol.code).toBe('B');
    expect(r.days[0].boards.two_board[0].symbol.code).toBe('C');
    // 1 板不进天梯
    expect(r.days[0].boards.two_board.find(x => x.symbol.code === 'A')).toBeUndefined();
  });
...
describe('SDK 专属扩展能力', () => {
  ...
  it('北向：分时 / 汇总 / 持股排行 / 历史', async () => {
    ...
    await s.getNorthboundHistory({ direction: 'north', startMs: new Date('2024-01-01').getTime(), endMs: new Date('2024-01-31').getTime() });
    ...
    expect(mockSdk.northbound.history).toHaveBeenCalledWith('north', expect.objectContaining({}));
  });
  it('盘口异动 / 板块异动 / 个股异动历史', async () => {
    ...
    const s = new StockSdkSource();
    await s.getStockChangeEvents(['rocket_launch'] as any);
  ...
  it('龙虎榜：个股统计 / 机构 / 营业部 / 席位', async () => {
    ...
    await s.getDragonTigerSeatDetail({ symbol: CN('600519'), date: '2024-01-02' });
    expect(mockSdk.dragonTiger.stockStats).toHaveBeenCalledWith('1month');
    expect(mockSdk.dragonTiger.institution).toHaveBeenCalledWith({ startDate: '20240101', endDate: '20240131' });
  ...
  it('概念板块 / 行业板块行情', async () => {
    ...
    await s.getConceptBoardConstituents({ code: 'BK0001', exchange: 'SH' });
  ...
  it('交易日历工具：isTradingDay / nextTradingDay / prevTradingDay / marketStatus', async () => {
    ...
    expect(await s.getMarketStatus('HK')).toBe('closed');
  });
...
describe('新增 API：板块 K 线 / 行情补充 / 北向个股 / 个股异动', () => {
  it('getIndustryBoardKline 委托 board.industry.kline', async () => {
    mockSdk.board.industry.kline.mockResolvedValue([{ date: '2024-01-01', open: 1, high: 2, low: 0.5, close: 1.5, volume: 100 }]);
    const s = new StockSdkSource();
    await s.getIndustryBoardKline({ symbol: { code: 'BK0001', exchange: 'SH' }, period: 'weekly' });
    expect(mockSdk.board.industry.kline).toHaveBeenCalledWith('BK0001', expect.objectContaining({ period: 'weekly' }));
  });
  ...
  it('getLargeOrders 委托 quotes.largeOrder', async () => {
    ...
    const s = new StockSdkSource();
    const r = await s.getLargeOrderRatios([CN('600519')]);
  ...
  it('getSectorFundFlowHistory 委托 fundFlow.sectorHistory', async () => {
    ...
    const s = new StockSdkSource();
    await s.getSectorFundFlowHistory({ symbol: { code: 'BK0438', exchange: 'SH' }, period: 'daily' });
  ...
  it('getNorthboundIndividual 委托 northbound.individual', async () => {
    ...
    const s = new StockSdkSource();
    await s.getNorthboundIndividual({ symbol: CN('000001'), startMs: new Date('2024-01-01').getTime(), endMs: new Date('2024-01-31').getTime() });
    expect(mockSdk.northbound.individual).toHaveBeenCalledWith('000001', expect.objectContaining({}));
  });
  it('getIndividualChangeEvents 委托 marketEvent.individualChanges', async () => {
    mockSdk.marketEvent.individualChanges.mockResolvedValue([{ code: '600519' }]);
    const s = new StockSdkSource();
    await s.getIndividualChangeEvents({ symbol: CN('600519'), date: '2024-01-02' });
    expect(mockSdk.marketEvent.individualChanges).toHaveBeenCalledWith('600519', { date: '20240102' });
  });
});
...
describe('新增 API：期权命名空间 options.*', () => {
  ...
  it('getOptionLhb 委托 options.lhb', async () => {
    ...
    const s = new StockSdkSource();
    await s.getOptionLhb({ symbol: { code: '10004336', exchange: 'SH' }, date: '2024-08-08' });
    expect(mockSdk.options.lhb).toHaveBeenCalledWith('10004336', '20240808');
  });
});
...
describe('新增 API：期货命名空间 futures.*', () => {
  it('getFuturesKline 委托 futures.kline', async () => {
    ...
    const s = new StockSdkSource();
    await s.getFuturesKline({ code: 'MA0', period: 'daily', startMs: new Date('2024-01-01').getTime(), endMs: new Date('2024-01-31').getTime() });
    expect(mockSdk.futures.kline).toHaveBeenCalledWith('MA0', expect.objectContaining({ period: 'daily' }));
  });
  it('getFuturesGlobalKline 委托 futures.globalKline', async () => {
    ...
    const s = new StockSdkSource();
    await s.getFuturesGlobalKline({ code: 'GLNC', period: 'weekly' });
    expect(mockSdk.futures.globalKline).toHaveBeenCalledWith('GLNC', expect.objectContaining({ period: 'weekly' }));
  });
  it('getFuturesInventorySymbols 委托 futures.inventorySymbols', async () => {
    mockSdk.futures.inventorySymbols.mockResolvedValue([{ code: 'CU', name: '铜', marketCode: '1' }]);
    const s = new StockSdkSource();
    const r = await s.getFuturesInventorySymbols();
    expect(mockSdk.futures.inventorySymbols).toHaveBeenCalled();
    expect(r[0].code).toBe('CU');
  });
  it('getFuturesInventory 委托 futures.inventory', async () => {
    mockSdk.futures.inventory.mockResolvedValue([{ date: '2024-01-02', inventory: 100 }]);
    const s = new StockSdkSource();
    await s.getFuturesInventory({ code: 'CU', startMs: new Date('2024-01-01').getTime() });
    expect(mockSdk.futures.inventory).toHaveBeenCalledWith('CU', expect.objectContaining({}));
  });
  it('getFuturesComexInventory 委托 futures.comexInventory', async () => {
    mockSdk.futures.comexInventory.mockResolvedValue([{ date: '2024-01-02', storageTon: 100 }]);
    const s = new StockSdkSource();
    await s.getFuturesComexInventory({ metal: 'gold' });
    expect(mockSdk.futures.comexInventory).toHaveBeenCalledWith('gold');
  });
});
